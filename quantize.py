...
def hist_dist_compared(path, qb, qsamples, qsamples2):
    ...
    # plt.bar(x[:-1], y, align='center', label='Sample from Normal distr. quantized on ' + str(qb) + ' bits')
    # plt.bar(x[:-1], y2, align='center', label='Empirical distr. quantized on ' + str(qb) + ' bits', width=0.5)
    plt.bar(x[:-1], y, align='center', label='Normal Distribution', color='tab:blue')
    plt.bar(x[:-1], y2, align='center', label='Empirical Distribution', width=0.5, color='tab:orange')
    plt.gca().set_xticks(x[:-1])
    plt.title("Histogram of quantized increments")
    plt.title("E80.0", fontsize=16)
    plt.xlabel("Quantized increment value", fontsize=16)
    ...
    plt.savefig(os.path.join(path, "hist_quant_incr_VS_sample_" + str(qb) + ".pdf"), bbox_inches='tight')
    plt.close()
