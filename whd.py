...
def whd_matrix(workdir, unneeded=[], colnames=[], dst_folder='', q_amp=8):
    ...
    for file in os.listdir(workdir):
        if not file.startswith("mean") and (file.endswith('.csv') or file.endswith('.h5')):
...
def cross_whd_matrix(reference, experiment, unneeded=[], colnames=[], q_amp=8):
    if len(os.listdir(reference)) == 0 or len(os.listdir(experiment)) == 0:
        return [[0]], [[0]]
    ...
    m_stddev = [[0 for _ in range(len(dfj))] for _ in range(len(dfi))]
    m_mean = [[0 for _ in range(len(dfj))] for _ in range(len(dfi))]
    normfact = len(colnames) * ((2 ** q_amp) - 1)
